...
(() => {
    ...
    function collectFromLineIds() {
        const lineNodes = Array.from(document.querySelectorAll("[id^='LC']"))
    ...
    function getPreviewChromeStyle() {
        return [
            "<style>",
            "html{background:#fff;}",
    ...
    function getDirectoryUrl(fileUrl) {
    ...
    function getRawAssetBaseUrl(pageUrl, rawFileUrl) {
        if (rawFileUrl) {
    ...
    function withAssetBase(payload) {
  ...
  `;
    ...
    function findControlByText(scope, label) {
        ...
        const labelElement = Array.from(scope.querySelectorAll("span, div, strong"))
            .find((element) => normalizeText(element.textContent) === label);
        return labelElement?.closest("button, a, [role='button']") ?? null;
    }
    function findFileToolbar() {
        const firstLine = document.querySelector("#LC1");
    ...
    function findCodeButton(scope = document) {
    ...
    function findInsertionTarget() {
        ...
        const filePanel = codeSurface?.closest(".Box, [class*='Box'], div");
    ...
    function findCodeSurface() {
        ...
        return (document.querySelector(".react-code-lines") ||
            document.querySelector(".js-file-line-container") ||
            document.querySelector("[data-testid='code-cell']")?.parentElement ||
            null);
    }
    function looksLikeFileToolbar(element) {
        if (!element) {
            return false;
        }
    ...
    function findFileBodyFromLine(line) {
        let current = line;
        while (current?.parentElement && current.parentElement !== document.body) {
            const previous = current.previousElementSibling;
            const parentPrevious = current.parentElement.previousElementSibling;
        ...
        return line.closest("table, .js-file-line-container, .react-code-lines");
    }
    function createPreviewFrame(payload) {
    ...
    function setPreviewButtonState(button) {
